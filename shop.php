...
?>
    ...
    <!-- Horizontal Filters Toolbar Above Catalog Line -->
    <div class="container mt-4">
        <div class="glass-card p-3 p-md-4 shadow-luxury">
            
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 pb-2 border-bottom border-secondary border-opacity-50">
                <div class="d-flex align-items-center gap-2">
                    <h5 class="font-heading text-white m-0"><i class="bi bi-sliders text-warning me-2"></i>Filter Catalog</h5>
                    <span class="badge bg-gold-subtle text-warning px-2 py-1 small"><?php echo count($productsList); ?> Fragrances</span>
                </div>
                <div>
                    <a href="shop.php" class="small text-warning text-decoration-none hover-gold"><i class="bi bi-arrow-counterclockwise me-1"></i>Reset Filters</a>
                </div>
            </div>

            <!-- Horizontal Filter Form Controls -->
            <form method="GET" action="shop.php" class="row g-3 align-items-end">
                
                <!-- Keyword Search Input -->
                <div class="col-12 col-md-3">
                    <label class="form-label text-white-50 small mb-1 fw-medium">Search Fragrance</label>
                    <div class="input-group input-group-sm">
                        <input type="text" name="search" class="form-control bg-dark border-secondary text-white" placeholder="Search perfume name..." value="<?php echo htmlspecialchars($searchKeyword); ?>">
                        <button class="btn btn-gold px-3" type="submit"><i class="bi bi-search"></i></button>
                    </div>
                </div>

                <!-- Fragrance Category Dropdown -->
                <div class="col-6 col-md-2">
                    <label class="form-label text-white-50 small mb-1 fw-medium">Category</label>
                    <select name="category" class="form-select form-select-sm bg-dark border-secondary text-white" onchange="this.form.submit()">
                        <option value="">All Categories</option>
                        <?php foreach ($categoriesList as $cat): ?>
                            <option value="<?php echo $cat['slug']; ?>" <?php echo ($filterCategory === $cat['slug']) ? 'selected' : ''; ?>>
                                <?php echo $cat['name']; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-6 col-md-2">
                    <label class="form-label text-white-50 small mb-1 fw-medium">Perfume House</label>
                    <select name="brand" class="form-select form-select-sm bg-dark border-secondary text-white" onchange="this.form.submit()">
                        <option value="">All Brands</option>
                        <?php foreach ($brandsList as $br): ?>
                            <option value="<?php echo $br['slug']; ?>" <?php echo ($filterBrand === $br['slug']) ? 'selected' : ''; ?>>
                                <?php echo $br['name']; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <!-- Bottle Size -->
                <div class="col-6 col-md-2">
                    <label class="form-label text-white-50 small mb-1 fw-medium">Bottle Size</label>
                    <select name="size" class="form-select form-select-sm bg-dark border-secondary text-white" onchange="this.form.submit()">
                        <option value="">All Sizes</option>
                        <option value="30ml" <?php echo ($filterSize === '30ml') ? 'selected' : ''; ?>>30ml</option>
                        <option value="50ml" <?php echo ($filterSize === '50ml') ? 'selected' : ''; ?>>50ml</option>
                        <option value="100ml" <?php echo ($filterSize === '100ml') ? 'selected' : ''; ?>>100ml</option>
                    </select>
                </div>

                <!-- Sorting Dropdown -->
                <div class="col-6 col-md-3">
                    <label class="form-label text-white-50 small mb-1 fw-medium">Sort By</label>
                    <select name="sort_by" class="form-select form-select-sm bg-dark border-secondary text-white" onchange="this.form.submit()">
                ...
                <!-- Price Range Slider & Options Row -->
                <div class="col-12 pt-3 mt-3 border-top border-secondary border-opacity-25">
                    <div class="row align-items-center g-3">
                        <div class="col-12 col-md-6">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <span class="text-white-50 small fw-medium">Max Price (INR):</span>
                                <strong class="text-warning small"><?php echo $settings['currency_symbol'] ?? '₹'; ?><span id="topPriceVal"><?php echo (int)($filterMaxPrice > 0 ? $filterMaxPrice : 25000); ?></span></strong>
                            </div>
                            <input type="range" class="form-range" name="max_price" id="topMaxPriceSlider" min="500" max="25000" step="500" value="<?php echo (int)($filterMaxPrice > 0 ? $filterMaxPrice : 25000); ?>" oninput="document.getElementById('topPriceVal').innerText=this.value">
                        </div>
                        <div class="col-6 col-md-3">
                            <div class="form-check form-switch m-0">
                                <input class="form-check-input bg-dark border-secondary" type="checkbox" name="availability" value="1" id="stockToggleTop" <?php echo ($filterAvailability == 1) ? 'checked' : ''; ?> onchange="this.form.submit()">
                                <label class="form-check-label text-white-50 small ms-1" for="stockToggleTop">In Stock Only</label>
                            </div>
                        </div>
                        <div class="col-6 col-md-3 text-end">
                            <button type="submit" class="btn btn-sm btn-gold px-4 py-2 fw-bold shadow-gold w-100 w-md-auto">
    ...
    <div class="container py-4">
        ...
        <!-- Products Grid (Full 12-Column Layout) -->
        <div class="row g-4">
